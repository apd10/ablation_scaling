"""k=3 fits: expert_size (y) vs layer_size (x), layer_size = total params / num_layers."""
...
def load_expert_layer_data():
    df = exclude_fit_outliers(load_survey())
    return df[(df["expert_size"] > 0) & (df["layer_size_m"] > 0)].copy()
...
def save_plot(x: np.ndarray, y: np.ndarray, result: FitResult, out: Path) -> None:
    ...
    ax.set_title(
        f"k={result.k} expert_size vs layer_size ({FIT_LABEL[result.x_axis_type]})"
...
def main() -> None:
    df = load_expert_layer_data()
    x = df["layer_size_m"].to_numpy()
    y = df["expert_size"].to_numpy()
    print(f"Models with expert_size > 0: {len(df)}")
    ...
    for axis_type in AXIS_TYPES:
        ...
        out = FIG_DIR / f"expert_size_vs_layer_size_k{KMEANS_NUM_FITS}_{axis_type}.png"
